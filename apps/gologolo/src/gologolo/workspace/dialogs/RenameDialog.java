...
import static gologolo.GoLogoPropertyType.GL_DIALOG_NAME_CANCEL_BUTTON;
import static gologolo.GoLogoPropertyType.GL_DIALOG_NAME_HEADER;
import static gologolo.GoLogoPropertyType.GL_DIALOG_NAME_HEADER_TEXT;
import static gologolo.GoLogoPropertyType.GL_DIALOG_NAME_OK_BUTTON;
import static gologolo.GoLogoPropertyType.GL_DIALOG_NAME_PROMPT;
...
public class RenameDialog extends Stage {
    ...
    public RenameDialog(GologoloApp initApp) {
    ...
    private void initDialog() {
        ...
        initGridNode(headerLabel,     GL_DIALOG_NAME_HEADER,          CLASS_GLL_DIALOG_HEADER,       0, 0, 3, 1, true);
        initGridNode(nameLabel,     GL_DIALOG_NAME_PROMPT,   CLASS_GLL_DIALOG_PROMPT,       0, 1, 1, 1, true);
        initGridNode(nameTextField, null,                      CLASS_GLL_DIALOG_TEXT_FIELD,   1, 1, 1, 1, false);
        initGridNode(okCancelPane,    null,                      CLASS_GLL_DIALOG_PANE,         0, 3, 2, 1, false);

        okButton = new Button();
        okButton.setDisable(false);
        cancelButton = new Button();
        app.getGUIModule().addGUINode(GL_DIALOG_NAME_OK_BUTTON, okButton);
        app.getGUIModule().addGUINode(GL_DIALOG_NAME_CANCEL_BUTTON, cancelButton);
...
//        widthTextField.setPromptText("(necessary)");
        ...
        AppLanguageModule languageSettings = app.getLanguageModule();
        languageSettings.addLabeledControlProperty(GL_DIALOG_NAME_OK_BUTTON + "_TEXT",    okButton.textProperty());
        languageSettings.addLabeledControlProperty(GL_DIALOG_NAME_CANCEL_BUTTON + "_TEXT",    cancelButton.textProperty());
...
//        });
    ...
    public void showRenameDialog(String oldname) {        
        ...
        String headerText = props.getProperty(GL_DIALOG_NAME_HEADER_TEXT);
    ...
    private void processCompleteWork() {
        new_name = nameTextField.getText();
        this.hide();
